	 ...
	 int *x_lookup)
{
    ...
    //simResults[4] += get_comp_counter();
    //simCounts[4]++;
    //4--------------------------------------------------------------------------------------------
    start_comp_counter();
    
    //average loops size
    //simResults[5] += num_nets_affected;
    //simCounts[5]++;
    
    if(num_nets_affected > 1)
    {
		for(k = 0; k < num_nets_affected - 1; k+=2)
		{
	
			
			inet1 = nets_to_update[k];
			...
			if(net_block_moved[k] == FROM_AND_TO && net_block_moved[k+1] == FROM_AND_TO)
			{
				continue;
			}
			else if((net_block_moved[k] == FROM_AND_TO && net_block_moved[k+1] != FROM_AND_TO)
					|| (net_block_moved[k] != FROM_AND_TO && net_block_moved[k+1] == FROM_AND_TO))
			{
			...
			{
				if(net[inet1].num_sinks < SMALL_NET)
				{
					get_non_updateable_bb(inet1, &bb_coord_new[bb_index]);
				}
				
				else//????????????????????????????????????????????////
				{
					if(net_block_moved[k] == FROM)
					update_bb(inet1, &bb_coord_new[bb_index],
						  &bb_edge_new[bb_index], x_from, y_from,
						  x_to, y_to);
					else
					update_bb(inet1, &bb_coord_new[bb_index],
					...
					if(net_block_moved[k+1] == FROM)
					update_bb(inet2, &bb_coord_new[bb_index+1],
						  &bb_edge_new[bb_index+1], x_from, y_from,
						  x_to, y_to);
					else
					update_bb(inet2, &bb_coord_new[bb_index+1],
						  &bb_edge_new[bb_index+1], x_to, y_to, x_from,
						  y_from);
				}
				
				if(net[inet2].num_sinks < SMALL_NET)
				{
					get_non_updateable_bb(inet2, &bb_coord_new[bb_index+1]);
				}
				...
				{
					temp_net_cost[inet1] =
					get_net_cost(inet1, &bb_coord_new[bb_index]);
					bb_delta_c += temp_net_cost[inet1] - net_cost[inet1];
					
					temp_net_cost[inet2] =
					get_net_cost(inet2, &bb_coord_new[bb_index+1]);
					bb_delta_c += temp_net_cost[inet2] - net_cost[inet2];
				}
				else
				{
					/* Rip up, then replace with new bb. */
					update_region_occ(inet1, &bb_coords[inet1], -1,
							  num_regions);
					update_region_occ(inet1, &bb_coord_new[bb_index], 1,
							  num_regions);
					
					update_region_occ(inet2, &bb_coords[inet2], -1,
							  num_regions);
					update_region_occ(inet2, &bb_coord_new[bb_index+1], 1,
							  num_regions);
				}
		...
		if(num_nets_affected - 1 == k)
		{
			...
			//simResults[10] += get_comp_counter();
			//simCounts[10]++;
			//10-------------------------------------------------------------------------------------------
			}
		}
    }
    else if(num_nets_affected == 1)
    {
			//--------------------------------------------------------------------------------------------
			//start_comp_counter();
			
			inet = nets_to_update[0];
			
			
			/* If we swapped two blocks connected to the same net, its bounding box *
			 * doesn't change.                                                      */
	
			if(net_block_moved[0] != FROM_AND_TO){
				//simResults[8] += get_comp_counter();
				//simCounts[8]++;
		   ...
		   float *delta_delay)
{
    ...
    //simResults[11] += get_comp_counter();
    //simCounts[11]++;
    //11--------------------------------------------------------------------------------------------
    //start_comp_counter();
    ...
    *delta_delay = delta_delay_cost;
    ...
    //simResults[12] += get_comp_counter();
    //simCounts[12]++;
    //12--------------------------------------------------------------------------------------------
}
