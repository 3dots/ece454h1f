 ...
 **********************************************************/
 int mm_init(void)
 {
   if ((heap_listp = mem_sbrk(6*WSIZE)) == (void *)-1)
     ...
     PUT(heap_listp + (2 * WSIZE), NULL);				// prologue pointer to prev
     PUT(heap_listp + (3 * WSIZE), NULL);				// prologue pointer to next
...
void *extend_heap(size_t words)
{
    ...
    PUT(NEXT_BLKP(bp), GET_NEXT_BLKP(heap_listp));	// Next element is the start of the linked list. (Or NULL if it was empy)
    PUt(PREV_BLKP(bp), NULL)						// Alwayes placing it at the front
    PUT(FTRP(bp), PACK(size, 0));              		// free block footer
    
    PUT(NEXT_BLKP(heap_listp), bp);					//Make the Head point to this newly allocated block
...
void * find_fit(size_t asize)
{
    ...
    for (bp = GET_NEXT_BLKP(heap_listp); bp != NULL; bp = GET_NEXT_BLKP(bp))
    {
        if ( asize <= GET_SIZE(HDRP(bp)) ) //No need to check that not allocated
        {
            return bp;
        }
    }
